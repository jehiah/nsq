...
var TopologyChannel = Backbone.Model.extend({
    idAttribute: 'name',

    constructor: function Topology() {
        Backbone.Model.prototype.constructor.apply(this, arguments);
    },

    url: function() {
        return AppState.apiPath('/topology/topics/' +
    ...
    parse: function(response) {
        response['nodes'] = _.map(response['nodes'] || [], function(node) {
            ...
            var hostname = node['node_stats'].hostname;
            node['show_broadcast_address'] = hostname.toLowerCase() !== address.toLowerCase();
            node['hostname_port'] = hostname + ':' + port;
            node['node_stats']['clients'] = _.map(node['node_stats']['clients'] || [], function(client) {
...
module.exports = TopologyChannel;
